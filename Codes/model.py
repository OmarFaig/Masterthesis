...
class FeatureExtractor(nn.Module):
    def __init__(self, out_dim=2048, n_knn=10):
        """Encoder that encodes information of partial point cloud
        """
        #in_channel values are not correct needs to be investigated
        super(FeatureExtractor, self).__init__()
        self.sa_module_1 = PointNet_SA_Layer(npoints=2000,nsample=16,in_channel=6,mlp_channels=[64,256] )
        self.transformer_1 = vTransformer(256, dim=64, n_knn=n_knn)
        self.sa_module_2 = PointNet_SA_Layer(npoints=1000,nsample=8,in_channel=259,mlp_channels=[256,512])
        self.transformer_2 = vTransformer(512, dim=128, n_knn=n_knn)
        self.sa_module_3 = PointNet_SA_Layer(npoints=None,nsample=None,in_channel=515,mlp_channels=[512,out_dim])
