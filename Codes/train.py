...
model = seedformer_dim128(up_factors=[1, 2, 4])  # 512 1024 4096 9192
...
dataloader = DataLoader(dataset, batch_size=16, shuffle=True, num_workers=8, collate_fn=utils.collate_fn)
...
checkpoint_path = 'training_1305v002.pth'
...
for epoch in range(start_epoch, epochs):
    ...
    with tqdm(enumerate(dataloader, 0), total=len(dataloader), desc=f'Epoch {epoch + 1}/{epochs}',
        ...
        for i, data in pbar:
            ...
            pbar.set_postfix(loss=loss_total)

        scheduler.step(metrics=loss_total)  # Step the learning rate scheduler
    ...
    logging.info(
        f'Epoch {epoch + 1} Loss: {loss_total, avg_cd1, avg_cd2, avg_cd3, avg_partial} LR: {last_lr}')  # Log it into the file
